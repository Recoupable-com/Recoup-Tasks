...
export async function registerOrgSubmodules(
  ...
  const message = [
    ...
    "2. Remove any existing entry (plain tree or submodule) from the git index:",
    "   git rm -r --cached .openclaw/workspace/orgs/{name} 2>/dev/null || true",
    "3. Remove stale submodule config: git config --remove-section submodule..openclaw/workspace/orgs/{name} 2>/dev/null || true",
    "4. Remove stale module cache: rm -rf .git/modules/.openclaw/workspace/orgs/{name} 2>/dev/null || true",
    "5. Remove the working tree copy: rm -rf .openclaw/workspace/orgs/{name} 2>/dev/null || true",
    "6. Run: git submodule add <remote-url> .openclaw/workspace/orgs/{name}",
    "   - Use GITHUB_TOKEN env var for auth: replace https://github.com/ with https://x-access-token:$GITHUB_TOKEN@github.com/",
    "7. After ALL submodules are added, strip x-access-token auth from .gitmodules so tokens are not committed",
    "8. Re-stage .gitmodules: git add .gitmodules",
    "",
    "IMPORTANT:",
